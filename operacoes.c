...
void RspU(long instruction, long Rx, long Ry, long Rz){
	...
	switch (instruction){
		...
		case 4:
			...
			else if((R[Rx]*R[Ry])>0x7FFFFFFF||(aux1*aux2)>0xFFFFFFFF)
			{
				if(R[35]<0x10)
					R[35]=R[35]+0x00000010;
			}
...
void RspF(long instruction, long IM16, long Rx, long Ry){
	unsigned long aux1, aux2;
	int aux3, aux4, aux5;
	switch (instruction){
		//addi
...
void RspS(long instruction, long S){
	switch (instruction){
	//bun
	case 26:
		if(S==0)
...
//funcao recursiva para deslocamento para direita
void shrvalue(long Rz,long Rx, long Ry){
	if(R[34]!=0){
		R[34]=R[34]>>(Ry+1);
		R[Rz]=R[Rx]<<(Ry+1);
	}
...
//funcao recursiva para deslocamento a direita
void shlvalue(long Rz, long Rx, long Ry){
	if((R[Rx]<<Ry)>LONG_MAX){
		R[34]=(R[34]<<1)+((R[Rx]&0x80000000)>>31);
		R[Rx]=(0x7FFFFFFF&R[Rx])<<1;
		Ry=Ry-1;
		shlvalue(Rz,Rx,Ry);
	}
	else
	...
	while(R[32]<j){
		...
		if(wholeWord[R[32]]==0)
			R[32]++;
				...
				printf("%d %s saida: %x Rz: %x R%d: %x R%x: %x \n", i,parse,R[U[1]],U[1],U[2], R[U[2]],U[3],R[U[3]]);
				i++;
		}
				...
				printf("%d %s saida: %x Rx: %x Ry: %x IM16 %x \n", i,parse,R[F[1]],R[F[1]],R[F[2]], F[0]);
				i++;
		}
