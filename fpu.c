...
unsigned int stopme;
unsigned int R[64];
unsigned int x, y, zmem;
unsigned int t_age;
float z;
unsigned int controlFpu;
unsigned int counter_data, counter_hits_data;
unsigned int counter_instr, counter_hits_instr;
...
void instr_cache(unsigned int PC, unsigned int op, unsigned int* wholeWord, unsigned int tamanho){
	...
	int l, hit, bloco, valido;
	counter_instr++;
	for(l=0; l<8; l++){
		if(cache_instr[l][0]==1){
			cache_instr[l][1]++;
		}
		if(cache_instr[l][7]==1){
			cache_instr[l][8]++;
		}
	}
	//0 leitura, 1 escrita
	if(op==0){
		fprintf(saida, "[CACHE D LINE %d READ", linha);
	}
	else{
		fprintf(saida, "[CACHE D LINE %d WRITE", linha);
	}
	//Hit Checker
	if(((cache_instr[linha][0]==1)&&(id_p==cache_instr[linha][2]))){
		...
		hit=0;
		fprintf(saida, "MISS @ 0x%08X]\n", PC);
	}
	fprintf(saida,"[SET 0: ");
	if(cache_instr[linha][0]==1){
		fprintf(saida, "VALID, AGE %d, DATA 0x%08X 0x%08X 0x%08X 0x%08X]\n",cache_instr[linha][1], cache_instr[linha][3]<<2, cache_instr[linha][4]<<2, cache_instr[linha][5]<<2, cache_instr[linha][6]<<2);
	}
	else{
		fprintf(saida, "INVALID, AGE %d, DATA 0x%08X 0x%08X 0x%08X 0x%08X]\n",cache_instr[linha][1], cache_instr[linha][3], cache_instr[linha][4], cache_instr[linha][5], cache_instr[linha][6]);
	}
	fprintf(saida,"[SET 1: ");
	if(cache_instr[linha][7]==1){
		fprintf(saida, "VALID, AGE %d, DATA 0x%08X 0x%08X 0x%08X 0x%08X]\n",cache_instr[linha][8], cache_instr[linha][10]<<2, cache_instr[linha][11]<<2, cache_instr[linha][12]<<2, cache_instr[linha][13]<<2);
	}
	else{
		fprintf(saida, "INVALID, AGE %d, DATA 0x%08X 0x%08X 0x%08X 0x%08X]\n",cache_instr[linha][8], cache_instr[linha][10], cache_instr[linha][11], cache_instr[linha][12], cache_instr[linha][13]);
	}
	if(op==1 && hit == 1){
		if(bloco == 0){
			cache_instr[linha][n_palavra+3]=wholeWord[PC];	
		}
		else{
			cache_instr[linha][n_palavra+10]=wholeWord[PC];
		}
	}
	else if(op==0 && hit ==0){
		for(l=0; l<4; l++){
...
void dado_cache(unsigned int PC, unsigned int op, unsigned int* wholeWord, unsigned int tamanho){
	...
	counter_data++;
	for(l=0; l<8; l++){
		if(cache_dados[l][0]==1){
			cache_dados[l][1]++;
		}
		if(cache_dados[l][7]==1){
			cache_dados[l][8]++;
		}
	}
	//0 leitura, 1 escrita
	...
	fprintf(saida,"[SET 0: ");
	if(cache_dados[linha][0]==1){
		fprintf(saida, "VALID, AGE %d, DATA 0x%08X 0x%08X 0x%08X 0x%08X]\n",cache_dados[linha][1], cache_dados[linha][3]<<2, cache_dados[linha][4]<<2, cache_dados[linha][5]<<2, cache_dados[linha][6]<<2);
	}
	else{
		fprintf(saida, "INVALID, AGE %d, DATA 0x%08X 0x%08X 0x%08X 0x%08X]\n",cache_dados[linha][1], cache_dados[linha][3], cache_dados[linha][4], cache_dados[linha][5], cache_dados[linha][6]);
	}
	fprintf(saida,"[SET 1: ");
	if(cache_dados[linha][7]==1){
		fprintf(saida, "VALID, AGE %d, DATA 0x%08X 0x%08X 0x%08X 0x%08X]\n",cache_dados[linha][8], cache_dados[linha][10]<<2, cache_dados[linha][11]<<2, cache_dados[linha][12]<<2, cache_dados[linha][13]<<2);
	}
	else{
		fprintf(saida, "INVALID, AGE %d, DATA 0x%08X 0x%08X 0x%08X 0x%08X]\n",cache_dados[linha][8], cache_dados[linha][10], cache_dados[linha][11], cache_dados[linha][12], cache_dados[linha][13]);
	}
	if(op==1 && hit == 1){
		if(bloco == 0){
			cache_dados[linha][n_palavra+3]=wholeWord[PC];	
		}
		else{
			cache_dados[linha][n_palavra+10]=wholeWord[PC];
		}
	}
	else if(op==0 && hit ==0){
		...
		for(l=0; l<4; l++){
			...
			else if(cache_dados[linha][7]==0){
				if(PC<tamanho){
					cache_dados[linha][l+10]=wholeWord[PC];
				}
				else{
					PC=0;
					cache_dados[linha][l+10]=wholeWord[PC];
			...
			else if(cache_dados[linha][2]>cache_dados[linha][9]){
				if(PC<tamanho){
					cache_dados[linha][l+3]=wholeWord[PC];
				}
				else{
